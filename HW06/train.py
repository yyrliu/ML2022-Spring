...
def discriminator_train_one_step(
    discriminator, generator, r_imgs, opt, loss_fn, device
):
...
def train(overwrite=False):
    ...
    for epoch in range(cfg.config.n_epoch):
        ...
        for data in progress_bar:
            loss_d = discriminator_train_one_step(
                discriminator, generator, data, opt_D, loss_fn_d, device
            )
            if steps % cfg.config.n_critic == 0:
                loss_g = generator_train_one_step(
                    generator, discriminator, opt_G, loss_fn_g, device
                )
            steps += 1
        ...
        torchvision.utils.save_image(
            f_imgs_sample,
            f"{cfg.config.workspace_dir}/epoch_{epoch+1:02d}.jpg",
            nrow=10,
        )
