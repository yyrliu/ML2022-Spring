...
def comfirm_overwrite(patterns, overwrite):
    ...
    patterns = [
        str(pattern) if isinstance(pattern, Path) else pattern for pattern in patterns
    ]
    ...
    if len(old_records) > 0:
        if not overwrite:
            raise FileExistsError(
                f"Existing files in {[pattern for pattern in patterns]} found, Will not overwrite."
            )
        else:
            logger.warning(
